...
def _build_args(x_in__, g_in__, y_out_):
    __inputs__ = []
    for arg in [x_in__, g_in__]:
        if arg != None:
...
@tbe_register.register_operator("RmsNormAie", trans_bool_to_s8=False)
...
def rms_norm_aie(x_in__, g_in__, y_out_, kernel_name="rms_norm_aie", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, g_in__, y_out_)
    ...
    origin_func_name = "rms_norm_aie"
    ascendc_src_dir_ex = "rms_norm_aie"
    ascendc_src_dir = "rms_norm_aie"
    ascendc_src_file = "rms_norm_aie.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator RmsNormAie. kernel name is " + kernel_name)
    op_type = "RmsNormAie"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, g_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, g_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, g_in__, y_out_)
    result = check_op_cap("op_select_format", "RmsNormAie", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, g_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, g_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "RmsNormAie", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
