...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['hiddenStates', 'seqLen', 'ntokens']
...
def _build_args(hiddenStates_in__, seqLen_in__, ntokens_in__, outStates_out_):
    __inputs__ = []
    for arg in [hiddenStates_in__, seqLen_in__, ntokens_in__]:
        if arg != None:
...
@tbe_register.register_operator("UnpadInput", trans_bool_to_s8=False)
...
def unpad_input(hiddenStates_in__, seqLen_in__, ntokens_in__, outStates_out_, kernel_name="unpad_input", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(hiddenStates_in__, seqLen_in__, ntokens_in__, outStates_out_)
    ...
    origin_func_name = "unpad_input"
    ascendc_src_dir_ex = "unpad_input"
    ascendc_src_dir = "unpad_input"
    ascendc_src_file = "unpad_input.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator UnpadInput. kernel name is " + kernel_name)
    op_type = "UnpadInput"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[hiddenStates_in__, seqLen_in__, ntokens_in__], origin_outputs = [outStates_out_],\
...
def op_select_format(hiddenStates_in__, seqLen_in__, ntokens_in__, outStates_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(hiddenStates_in__, seqLen_in__, ntokens_in__, outStates_out_)
    result = check_op_cap("op_select_format", "UnpadInput", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(hiddenStates_in__, seqLen_in__, ntokens_in__, outStates_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(hiddenStates_in__, seqLen_in__, ntokens_in__, outStates_out_)
    result = check_op_cap("get_op_specific_info", "UnpadInput", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
