...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['newKV', 'layerId', 'CacheIn', 'tokenOffset', 'seqLen']
    output_names = ['cacheOut']
...
def _build_args(newKV_in__, layerId_in__, CacheIn_in__, tokenOffset_in__, seqLen_in__, cacheOut_out_, max_seqlen):
    __inputs__ = []
    for arg in [newKV_in__, layerId_in__, CacheIn_in__, tokenOffset_in__, seqLen_in__]:
        if arg != None:
...
@tbe_register.register_operator("KvCache", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def kv_cache(newKV_in__, layerId_in__, CacheIn_in__, tokenOffset_in__, seqLen_in__, cacheOut_out_, max_seqlen=32768, kernel_name="kv_cache", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(newKV_in__, layerId_in__, CacheIn_in__, tokenOffset_in__, seqLen_in__, cacheOut_out_, max_seqlen)
    ...
    origin_func_name = "kv_cache"
    ascendc_src_dir_ex = "kv_cache"
    ascendc_src_dir = "kv_cache"
    ascendc_src_file = "kv_cache.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator KvCache. kernel name is " + kernel_name)
    op_type = "KvCache"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[newKV_in__, layerId_in__, CacheIn_in__, tokenOffset_in__, seqLen_in__], origin_outputs = [cacheOut_out_],\
...
def op_select_format(newKV_in__, layerId_in__, CacheIn_in__, tokenOffset_in__, seqLen_in__, cacheOut_out_, max_seqlen=32768, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(newKV_in__, layerId_in__, CacheIn_in__, tokenOffset_in__, seqLen_in__, cacheOut_out_, max_seqlen)
    result = check_op_cap("op_select_format", "KvCache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(newKV_in__, layerId_in__, CacheIn_in__, tokenOffset_in__, seqLen_in__, cacheOut_out_, max_seqlen=32768, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(newKV_in__, layerId_in__, CacheIn_in__, tokenOffset_in__, seqLen_in__, cacheOut_out_, max_seqlen)
    result = check_op_cap("get_op_specific_info", "KvCache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
