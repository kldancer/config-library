...
def _build_args(x_in__, y_in__, z_in__, out_out_):
    __inputs__ = []
    for arg in [x_in__, y_in__, z_in__]:
        if arg != None:
...
@tbe_register.register_operator("Yulu", trans_bool_to_s8=False)
...
def yulu(x_in__, y_in__, z_in__, out_out_, kernel_name="yulu", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_in__, out_out_)
    ...
    origin_func_name = "yulu"
    ascendc_src_dir_ex = "yulu"
    ascendc_src_dir = "yulu"
    ascendc_src_file = "yulu.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Yulu. kernel name is " + kernel_name)
    op_type = "Yulu"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, y_in__, z_in__], origin_outputs = [out_out_],\
...
def op_select_format(x_in__, y_in__, z_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_in__, out_out_)
    result = check_op_cap("op_select_format", "Yulu", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_in__, z_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "Yulu", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
