...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'g', 'b', 'scale', 'offset']
    output_names = ['y']
...
def _build_args(x_in__, g_in__, b_in__, scale_in__, offset_in__, y_out_, eps, input_scale, input_offset, quant_min):
    __inputs__ = []
    for arg in [x_in__, g_in__, b_in__, scale_in__, offset_in__]:
        if arg != None:
    ...
    if input_scale != None:
        attr = {}
        attr["name"] = "input_scale"
        attr["dtype"] = "float"
        attr["value"] = input_scale
        __attrs__.append(attr)
    if input_offset != None:
        attr = {}
        attr["name"] = "input_offset"
        attr["dtype"] = "float"
        attr["value"] = input_offset
        __attrs__.append(attr)
    if quant_min != None:
        attr = {}
        attr["name"] = "quant_min"
        attr["dtype"] = "float"
        attr["value"] = quant_min
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("RmsNormQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def rms_norm_quant(x_in__, g_in__, b_in__, scale_in__=None, offset_in__=None, y_out_=None, eps=1e-05, input_scale=82.96, input_offset=-36, quant_min=-128, kernel_name="rms_norm_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, g_in__, b_in__, scale_in__, offset_in__, y_out_, eps, input_scale, input_offset, quant_min)
    ...
    origin_func_name = "rms_norm_quant"
    ascendc_src_dir_ex = "rms_norm_quant"
    ascendc_src_dir = "rms_norm_quant"
    ascendc_src_file = "rms_norm_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator RmsNormQuant. kernel name is " + kernel_name)
    op_type = "RmsNormQuant"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, g_in__, b_in__, scale_in__, offset_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'optional', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, g_in__, b_in__, scale_in__=None, offset_in__=None, y_out_=None, eps=1e-05, input_scale=82.96, input_offset=-36, quant_min=-128, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, g_in__, b_in__, scale_in__, offset_in__, y_out_, eps, input_scale, input_offset, quant_min)
    result = check_op_cap("op_select_format", "RmsNormQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, g_in__, b_in__, scale_in__=None, offset_in__=None, y_out_=None, eps=1e-05, input_scale=82.96, input_offset=-36, quant_min=-128, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, g_in__, b_in__, scale_in__, offset_in__, y_out_, eps, input_scale, input_offset, quant_min)
    result = check_op_cap("get_op_specific_info", "RmsNormQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
