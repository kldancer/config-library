...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['matrixA', 'matrixB', 'bias']
    output_names = ['matrixC']
...
def _build_args(matrixA_in__, matrixB_in__, bias_in__, matrixC_out_, transA, transB):
    __inputs__ = []
    for arg in [matrixA_in__, matrixB_in__, bias_in__]:
        if arg != None:
...
@tbe_register.register_operator("PpmatmulCube", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def ppmatmul_cube(matrixA_in__, matrixB_in__, bias_in__=None, matrixC_out_=None, transA=0, transB=0, kernel_name="ppmatmul_cube", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(matrixA_in__, matrixB_in__, bias_in__, matrixC_out_, transA, transB)
    ...
    origin_func_name = "ppmatmul_cube"
    ascendc_src_dir_ex = "ppmatmul_cube"
    ascendc_src_dir = "ppmatmul_cube"
    ascendc_src_file = "ppmatmul_cube.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator PpmatmulCube. kernel name is " + kernel_name)
    op_type = "PpmatmulCube"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[matrixA_in__, matrixB_in__, bias_in__], origin_outputs = [matrixC_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(matrixA_in__, matrixB_in__, bias_in__=None, matrixC_out_=None, transA=0, transB=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(matrixA_in__, matrixB_in__, bias_in__, matrixC_out_, transA, transB)
    result = check_op_cap("op_select_format", "PpmatmulCube", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(matrixA_in__, matrixB_in__, bias_in__=None, matrixC_out_=None, transA=0, transB=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(matrixA_in__, matrixB_in__, bias_in__, matrixC_out_, transA, transB)
    result = check_op_cap("get_op_specific_info", "PpmatmulCube", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
