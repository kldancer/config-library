...
def _build_args(x_in__, z_out_, scale, offset, quant_min):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("Quant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def quant(x_in__, z_out_, scale=82.96, offset=-36, quant_min=-128, kernel_name="quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, z_out_, scale, offset, quant_min)
    ...
    origin_func_name = "quant"
    ascendc_src_dir_ex = "quant"
    ascendc_src_dir = "quant"
    ascendc_src_file = "quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Quant. kernel name is " + kernel_name)
    op_type = "Quant"
...
def op_select_format(x_in__, z_out_, scale=82.96, offset=-36, quant_min=-128, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, z_out_, scale, offset, quant_min)
    result = check_op_cap("op_select_format", "Quant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, z_out_, scale=82.96, offset=-36, quant_min=-128, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, z_out_, scale, offset, quant_min)
    result = check_op_cap("get_op_specific_info", "Quant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
