...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['attenOut', 'Ori_IndiceA', 'Ori_IndiceB', 'TOPK_Indice', 'Arg_Max']
    output_names = ['unZipToken']
...
def _build_args(attenOut_in__, Ori_IndiceA_in__, Ori_IndiceB_in__, TOPK_Indice_in__, Arg_Max_in__, unZipToken_out_, topRRate):
    __inputs__ = []
    for arg in [attenOut_in__, Ori_IndiceA_in__, Ori_IndiceB_in__, TOPK_Indice_in__, Arg_Max_in__]:
        if arg != None:
...
@tbe_register.register_operator("TomeUnmerged", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def tome_unmerged(attenOut_in__, Ori_IndiceA_in__, Ori_IndiceB_in__, TOPK_Indice_in__, Arg_Max_in__, unZipToken_out_, topRRate=0.5, kernel_name="tome_unmerged", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(attenOut_in__, Ori_IndiceA_in__, Ori_IndiceB_in__, TOPK_Indice_in__, Arg_Max_in__, unZipToken_out_, topRRate)
    ...
    origin_func_name = "tome_unmerged"
    ascendc_src_dir_ex = "tome_unmerged"
    ascendc_src_dir = "tome_unmerged"
    ascendc_src_file = "tome_unmerged.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator TomeUnmerged. kernel name is " + kernel_name)
    op_type = "TomeUnmerged"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[attenOut_in__, Ori_IndiceA_in__, Ori_IndiceB_in__, TOPK_Indice_in__, Arg_Max_in__], origin_outputs = [unZipToken_out_],\
...
def op_select_format(attenOut_in__, Ori_IndiceA_in__, Ori_IndiceB_in__, TOPK_Indice_in__, Arg_Max_in__, unZipToken_out_, topRRate=0.5, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(attenOut_in__, Ori_IndiceA_in__, Ori_IndiceB_in__, TOPK_Indice_in__, Arg_Max_in__, unZipToken_out_, topRRate)
    result = check_op_cap("op_select_format", "TomeUnmerged", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(attenOut_in__, Ori_IndiceA_in__, Ori_IndiceB_in__, TOPK_Indice_in__, Arg_Max_in__, unZipToken_out_, topRRate=0.5, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(attenOut_in__, Ori_IndiceA_in__, Ori_IndiceB_in__, TOPK_Indice_in__, Arg_Max_in__, unZipToken_out_, topRRate)
    result = check_op_cap("get_op_specific_info", "TomeUnmerged", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
