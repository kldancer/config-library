...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['Q', 'Kcache', 'Vcache', 'AttentionMask']
...
def _build_args(Q_in__, Kcache_in__, Vcache_in__, AttentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_):
    __inputs__ = []
    for arg in [Q_in__, Kcache_in__, Vcache_in__, AttentionMask_in__]:
        if arg != None:
...
@tbe_register.register_operator("UnpadFlashAttentionMix", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def unpad_flash_attention_mix(Q_in__, Kcache_in__, Vcache_in__, AttentionMask_in__=None, outputO_out_=None, outputS_out_=None, outputP_out_=None, oTmp_out_=None, kernel_name="unpad_flash_attention_mix", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(Q_in__, Kcache_in__, Vcache_in__, AttentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_)
    ...
    origin_func_name = "unpad_flash_attention_mix"
    ascendc_src_dir_ex = "unpad_flash_attention_mix"
    ascendc_src_dir = "unpad_flash_attention_mix"
    ascendc_src_file = "unpad_flash_attention_mix.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator UnpadFlashAttentionMix. kernel name is " + kernel_name)
    op_type = "UnpadFlashAttentionMix"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[Q_in__, Kcache_in__, Vcache_in__, AttentionMask_in__], origin_outputs = [outputO_out_, outputS_out_, outputP_out_, oTmp_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'optional', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(Q_in__, Kcache_in__, Vcache_in__, AttentionMask_in__=None, outputO_out_=None, outputS_out_=None, outputP_out_=None, oTmp_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(Q_in__, Kcache_in__, Vcache_in__, AttentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_)
    result = check_op_cap("op_select_format", "UnpadFlashAttentionMix", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(Q_in__, Kcache_in__, Vcache_in__, AttentionMask_in__=None, outputO_out_=None, outputS_out_=None, outputP_out_=None, oTmp_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(Q_in__, Kcache_in__, Vcache_in__, AttentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_)
    result = check_op_cap("get_op_specific_info", "UnpadFlashAttentionMix", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
