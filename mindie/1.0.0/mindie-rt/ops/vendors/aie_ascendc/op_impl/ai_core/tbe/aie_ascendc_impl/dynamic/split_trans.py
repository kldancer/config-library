...
def _build_args(X_in__, Q_out_, K_out_, V_out_, Shape_out_):
    __inputs__ = []
    for arg in [X_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [Q_out_, K_out_, V_out_, Shape_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("SplitTrans", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def split_trans(X_in__, Q_out_, K_out_, V_out_, Shape_out_, kernel_name="split_trans", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(X_in__, Q_out_, K_out_, V_out_, Shape_out_)
    ...
    origin_func_name = "split_trans"
    ascendc_src_dir_ex = "split_trans"
    ascendc_src_dir = "split_trans"
    ascendc_src_file = "split_trans.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SplitTrans. kernel name is " + kernel_name)
    op_type = "SplitTrans"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[X_in__], origin_outputs = [Q_out_, K_out_, V_out_, Shape_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(X_in__, Q_out_, K_out_, V_out_, Shape_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(X_in__, Q_out_, K_out_, V_out_, Shape_out_)
    result = check_op_cap("op_select_format", "SplitTrans", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(X_in__, Q_out_, K_out_, V_out_, Shape_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(X_in__, Q_out_, K_out_, V_out_, Shape_out_)
    result = check_op_cap("get_op_specific_info", "SplitTrans", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
