...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['inputX', 'inputWeight', 'inputBias', 'inputExtra']
    output_names = ['outMatrix']
...
def _build_args(inputX_in__, inputWeight_in__, inputBias_in__, inputExtra_in__, outMatrix_out_, needTrans, withBias, operateType):
    __inputs__ = []
    for arg in [inputX_in__, inputWeight_in__, inputBias_in__, inputExtra_in__]:
        if arg != None:
    ...
    if operateType != None:
        attr = {}
        attr["name"] = "operateType"
        attr["dtype"] = "int"
        attr["value"] = operateType
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("MatmulAll", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def matmul_all(inputX_in__, inputWeight_in__, inputBias_in__=None, inputExtra_in__=None, outMatrix_out_=None, needTrans=0, withBias=0, operateType=0, kernel_name="matmul_all", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(inputX_in__, inputWeight_in__, inputBias_in__, inputExtra_in__, outMatrix_out_, needTrans, withBias, operateType)
    ...
    origin_func_name = "matmul_all"
    ascendc_src_dir_ex = "matmul_all"
    ascendc_src_dir = "matmul_all"
    ascendc_src_file = "matmul_all.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator MatmulAll. kernel name is " + kernel_name)
    op_type = "MatmulAll"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[inputX_in__, inputWeight_in__, inputBias_in__, inputExtra_in__], origin_outputs = [outMatrix_out_],\
...
def op_select_format(inputX_in__, inputWeight_in__, inputBias_in__=None, inputExtra_in__=None, outMatrix_out_=None, needTrans=0, withBias=0, operateType=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(inputX_in__, inputWeight_in__, inputBias_in__, inputExtra_in__, outMatrix_out_, needTrans, withBias, operateType)
    result = check_op_cap("op_select_format", "MatmulAll", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(inputX_in__, inputWeight_in__, inputBias_in__=None, inputExtra_in__=None, outMatrix_out_=None, needTrans=0, withBias=0, operateType=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(inputX_in__, inputWeight_in__, inputBias_in__, inputExtra_in__, outMatrix_out_, needTrans, withBias, operateType)
    result = check_op_cap("get_op_specific_info", "MatmulAll", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
