...
def _build_args(inputX_in__, outputX_out_):
    __inputs__ = []
    for arg in [inputX_in__]:
        if arg != None:
...
@tbe_register.register_operator("SliceTransGeluMul", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def slice_trans_gelu_mul(inputX_in__, outputX_out_, kernel_name="slice_trans_gelu_mul", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(inputX_in__, outputX_out_)
    ...
    origin_func_name = "slice_trans_gelu_mul"
    ascendc_src_dir_ex = "slice_trans_gelu_mul"
    ascendc_src_dir = "slice_trans_gelu_mul"
    ascendc_src_file = "slice_trans_gelu_mul.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SliceTransGeluMul. kernel name is " + kernel_name)
    op_type = "SliceTransGeluMul"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[inputX_in__], origin_outputs = [outputX_out_],\
...
def op_select_format(inputX_in__, outputX_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(inputX_in__, outputX_out_)
    result = check_op_cap("op_select_format", "SliceTransGeluMul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(inputX_in__, outputX_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(inputX_in__, outputX_out_)
    result = check_op_cap("get_op_specific_info", "SliceTransGeluMul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
