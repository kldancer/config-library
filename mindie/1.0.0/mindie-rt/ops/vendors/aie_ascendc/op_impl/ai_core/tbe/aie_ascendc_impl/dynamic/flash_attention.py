...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['query', 'key', 'value', 'seqLen', 'batch', 'spMask']
...
def _build_args(query_in__, key_in__, value_in__, seqLen_in__, batch_in__, spMask_in__, attnOut_out_):
    __inputs__ = []
    for arg in [query_in__, key_in__, value_in__, seqLen_in__, batch_in__, spMask_in__]:
        if arg != None:
...
@tbe_register.register_operator("FlashAttention", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def flash_attention(query_in__, key_in__, value_in__, seqLen_in__, batch_in__, spMask_in__, attnOut_out_, kernel_name="flash_attention", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, seqLen_in__, batch_in__, spMask_in__, attnOut_out_)
    ...
    origin_func_name = "flash_attention"
    ascendc_src_dir_ex = "flash_attention"
    ascendc_src_dir = "flash_attention"
    ascendc_src_file = "flash_attention.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator FlashAttention. kernel name is " + kernel_name)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[query_in__, key_in__, value_in__, seqLen_in__, batch_in__, spMask_in__], origin_outputs = [attnOut_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(query_in__, key_in__, value_in__, seqLen_in__, batch_in__, spMask_in__, attnOut_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, seqLen_in__, batch_in__, spMask_in__, attnOut_out_)
    result = check_op_cap("op_select_format", "FlashAttention", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(query_in__, key_in__, value_in__, seqLen_in__, batch_in__, spMask_in__, attnOut_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, seqLen_in__, batch_in__, spMask_in__, attnOut_out_)
    result = check_op_cap("get_op_specific_info", "FlashAttention", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
