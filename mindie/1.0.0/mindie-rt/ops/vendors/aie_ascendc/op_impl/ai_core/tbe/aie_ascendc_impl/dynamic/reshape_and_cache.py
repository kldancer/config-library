...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['keyInput', 'valueInput', 'keyCache', 'valueCache', 'slotMapping']
    output_names = ['keyCache', 'valueCache']
...
def _build_args(keyInput_in__, valueInput_in__, keyCache_in__, valueCache_in__, slotMapping_in__, keyCache_out_, valueCache_out_):
    __inputs__ = []
    for arg in [keyInput_in__, valueInput_in__, keyCache_in__, valueCache_in__, slotMapping_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [keyCache_out_, valueCache_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("ReshapeAndCache", trans_bool_to_s8=False)
...
def reshape_and_cache(keyInput_in__, valueInput_in__, keyCache_in__, valueCache_in__, slotMapping_in__, keyCache_out_, valueCache_out_, kernel_name="reshape_and_cache", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(keyInput_in__, valueInput_in__, keyCache_in__, valueCache_in__, slotMapping_in__, keyCache_out_, valueCache_out_)
    ...
    origin_func_name = "reshape_and_cache"
    ascendc_src_dir_ex = "reshape_and_cache"
    ascendc_src_dir = "reshape_and_cache"
    ascendc_src_file = "reshape_and_cache.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator ReshapeAndCache. kernel name is " + kernel_name)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[keyInput_in__, valueInput_in__, keyCache_in__, valueCache_in__, slotMapping_in__], origin_outputs = [keyCache_out_, valueCache_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(keyInput_in__, valueInput_in__, keyCache_in__, valueCache_in__, slotMapping_in__, keyCache_out_, valueCache_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(keyInput_in__, valueInput_in__, keyCache_in__, valueCache_in__, slotMapping_in__, keyCache_out_, valueCache_out_)
    result = check_op_cap("op_select_format", "ReshapeAndCache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(keyInput_in__, valueInput_in__, keyCache_in__, valueCache_in__, slotMapping_in__, keyCache_out_, valueCache_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(keyInput_in__, valueInput_in__, keyCache_in__, valueCache_in__, slotMapping_in__, keyCache_out_, valueCache_out_)
    result = check_op_cap("get_op_specific_info", "ReshapeAndCache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
