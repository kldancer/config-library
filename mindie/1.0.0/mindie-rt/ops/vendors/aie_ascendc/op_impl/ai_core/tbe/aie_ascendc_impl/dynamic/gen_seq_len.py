...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['attnMask']
    output_names = ['seqLenAlign', 'seqLenOri']
...
def _build_args(attnMask_in__, seqLenAlign_out_, seqLenOri_out_):
    __inputs__ = []
    for arg in [attnMask_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [seqLenAlign_out_, seqLenOri_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("GenSeqLen", trans_bool_to_s8=False)
...
def gen_seq_len(attnMask_in__, seqLenAlign_out_, seqLenOri_out_, kernel_name="gen_seq_len", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(attnMask_in__, seqLenAlign_out_, seqLenOri_out_)
    ...
    origin_func_name = "gen_seq_len"
    ascendc_src_dir_ex = "gen_seq_len"
    ascendc_src_dir = "gen_seq_len"
    ascendc_src_file = "gen_seq_len.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator GenSeqLen. kernel name is " + kernel_name)
    op_type = "GenSeqLen"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[attnMask_in__], origin_outputs = [seqLenAlign_out_, seqLenOri_out_],\
...
def op_select_format(attnMask_in__, seqLenAlign_out_, seqLenOri_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(attnMask_in__, seqLenAlign_out_, seqLenOri_out_)
    result = check_op_cap("op_select_format", "GenSeqLen", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(attnMask_in__, seqLenAlign_out_, seqLenOri_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(attnMask_in__, seqLenAlign_out_, seqLenOri_out_)
    result = check_op_cap("get_op_specific_info", "GenSeqLen", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
