...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['startNum', 'endNum', 'stepForward', 'dtypeOutput']
    output_names = ['outRange']
...
def _build_args(startNum_in__, endNum_in__, stepForward_in__, dtypeOutput_in__, outRange_out_):
    __inputs__ = []
    for arg in [startNum_in__, endNum_in__, stepForward_in__, dtypeOutput_in__]:
        if arg != None:
...
@tbe_register.register_operator("Arange", trans_bool_to_s8=False)
...
def arange(startNum_in__, endNum_in__, stepForward_in__, dtypeOutput_in__, outRange_out_, kernel_name="arange", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(startNum_in__, endNum_in__, stepForward_in__, dtypeOutput_in__, outRange_out_)
    ...
    origin_func_name = "arange"
    ascendc_src_dir_ex = "arange"
    ascendc_src_dir = "arange"
    ascendc_src_file = "arange.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Arange. kernel name is " + kernel_name)
    op_type = "Arange"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[startNum_in__, endNum_in__, stepForward_in__, dtypeOutput_in__], origin_outputs = [outRange_out_],\
...
def op_select_format(startNum_in__, endNum_in__, stepForward_in__, dtypeOutput_in__, outRange_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(startNum_in__, endNum_in__, stepForward_in__, dtypeOutput_in__, outRange_out_)
    result = check_op_cap("op_select_format", "Arange", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(startNum_in__, endNum_in__, stepForward_in__, dtypeOutput_in__, outRange_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(startNum_in__, endNum_in__, stepForward_in__, dtypeOutput_in__, outRange_out_)
    result = check_op_cap("get_op_specific_info", "Arange", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
