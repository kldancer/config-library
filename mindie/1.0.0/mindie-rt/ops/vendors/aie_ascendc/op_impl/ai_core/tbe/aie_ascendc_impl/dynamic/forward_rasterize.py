...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['face_vertices', 'out_shape']
...
def _build_args(face_vertices_in__, out_shape_in__, output_buffer_out_):
    __inputs__ = []
    for arg in [face_vertices_in__, out_shape_in__]:
        if arg != None:
...
@tbe_register.register_operator("ForwardRasterize", trans_bool_to_s8=False)
...
def forward_rasterize(face_vertices_in__, out_shape_in__, output_buffer_out_, kernel_name="forward_rasterize", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(face_vertices_in__, out_shape_in__, output_buffer_out_)
    ...
    origin_func_name = "forward_rasterize"
    ascendc_src_dir_ex = "forward_rasterize"
    ascendc_src_dir = "forward_rasterize"
    ascendc_src_file = "forward_rasterize.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator ForwardRasterize. kernel name is " + kernel_name)
    op_type = "ForwardRasterize"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[face_vertices_in__, out_shape_in__], origin_outputs = [output_buffer_out_],\
...
def op_select_format(face_vertices_in__, out_shape_in__, output_buffer_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(face_vertices_in__, out_shape_in__, output_buffer_out_)
    result = check_op_cap("op_select_format", "ForwardRasterize", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(face_vertices_in__, out_shape_in__, output_buffer_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(face_vertices_in__, out_shape_in__, output_buffer_out_)
    result = check_op_cap("get_op_specific_info", "ForwardRasterize", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
