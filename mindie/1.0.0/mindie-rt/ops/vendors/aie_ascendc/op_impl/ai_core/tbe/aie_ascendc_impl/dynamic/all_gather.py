...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['sendData', 'commArgs']
    output_names = ['recvData']
...
def _build_args(sendData_in__, commArgs_in__, recvData_out_, rank, rankSize, magicId):
    __inputs__ = []
    for arg in [sendData_in__, commArgs_in__]:
        if arg != None:
...
@tbe_register.register_operator("AllGather", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def all_gather(sendData_in__, commArgs_in__, recvData_out_, rank, rankSize, magicId, kernel_name="all_gather", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(sendData_in__, commArgs_in__, recvData_out_, rank, rankSize, magicId)
    ...
    origin_func_name = "all_gather"
    ascendc_src_dir_ex = "all_gather"
    ascendc_src_dir = "all_gather"
    ascendc_src_file = "all_gather.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator AllGather. kernel name is " + kernel_name)
    op_type = "AllGather"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[sendData_in__, commArgs_in__], origin_outputs = [recvData_out_],\
...
def op_select_format(sendData_in__, commArgs_in__, recvData_out_, rank, rankSize, magicId, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(sendData_in__, commArgs_in__, recvData_out_, rank, rankSize, magicId)
    result = check_op_cap("op_select_format", "AllGather", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(sendData_in__, commArgs_in__, recvData_out_, rank, rankSize, magicId, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(sendData_in__, commArgs_in__, recvData_out_, rank, rankSize, magicId)
    result = check_op_cap("get_op_specific_info", "AllGather", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
