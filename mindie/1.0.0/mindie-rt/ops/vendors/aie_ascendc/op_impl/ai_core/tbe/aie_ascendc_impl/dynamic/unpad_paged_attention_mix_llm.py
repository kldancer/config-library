...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['q', 'kCache', 'vCache', 'qSeqlen', 'kvSeqlen', 'blockTableGm', 'blockInfo', 'layerId', 'attentionMask']
    output_names = ['outputO', 'outputS', 'outputP', 'oTmp']
...
def _build_args(q_in__, kCache_in__, vCache_in__, qSeqlen_in__, kvSeqlen_in__, blockTableGm_in__, blockInfo_in__, layerId_in__, attentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_, kvHead, maxSeqlen):
    __inputs__ = []
    for arg in [q_in__, kCache_in__, vCache_in__, qSeqlen_in__, kvSeqlen_in__, blockTableGm_in__, blockInfo_in__, layerId_in__, attentionMask_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [outputO_out_, outputS_out_, outputP_out_, oTmp_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if maxSeqlen != None:
        ...
        attr["name"] = "maxSeqlen"
        attr["dtype"] = "int"
        attr["value"] = maxSeqlen
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("UnpadPagedAttentionMixLlm", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def unpad_paged_attention_mix_llm(q_in__, kCache_in__, vCache_in__, qSeqlen_in__, kvSeqlen_in__, blockTableGm_in__, blockInfo_in__, layerId_in__, attentionMask_in__=None, outputO_out_=None, outputS_out_=None, outputP_out_=None, oTmp_out_=None, kvHead=2, maxSeqlen=32768, kernel_name="unpad_paged_attention_mix_llm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, kCache_in__, vCache_in__, qSeqlen_in__, kvSeqlen_in__, blockTableGm_in__, blockInfo_in__, layerId_in__, attentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_, kvHead, maxSeqlen)
    ...
    origin_func_name = "unpad_paged_attention_mix_llm"
    ascendc_src_dir_ex = "unpad_paged_attention_mix_llm"
    ascendc_src_dir = "unpad_paged_attention_mix_llm"
    ascendc_src_file = "unpad_paged_attention_mix_llm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator UnpadPagedAttentionMixLlm. kernel name is " + kernel_name)
    op_type = "UnpadPagedAttentionMixLlm"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[q_in__, kCache_in__, vCache_in__, qSeqlen_in__, kvSeqlen_in__, blockTableGm_in__, blockInfo_in__, layerId_in__, attentionMask_in__], origin_outputs = [outputO_out_, outputS_out_, outputP_out_, oTmp_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'optional', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(q_in__, kCache_in__, vCache_in__, qSeqlen_in__, kvSeqlen_in__, blockTableGm_in__, blockInfo_in__, layerId_in__, attentionMask_in__=None, outputO_out_=None, outputS_out_=None, outputP_out_=None, oTmp_out_=None, kvHead=2, maxSeqlen=32768, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, kCache_in__, vCache_in__, qSeqlen_in__, kvSeqlen_in__, blockTableGm_in__, blockInfo_in__, layerId_in__, attentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_, kvHead, maxSeqlen)
    result = check_op_cap("op_select_format", "UnpadPagedAttentionMixLlm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(q_in__, kCache_in__, vCache_in__, qSeqlen_in__, kvSeqlen_in__, blockTableGm_in__, blockInfo_in__, layerId_in__, attentionMask_in__=None, outputO_out_=None, outputS_out_=None, outputP_out_=None, oTmp_out_=None, kvHead=2, maxSeqlen=32768, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, kCache_in__, vCache_in__, qSeqlen_in__, kvSeqlen_in__, blockTableGm_in__, blockInfo_in__, layerId_in__, attentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_, kvHead, maxSeqlen)
    result = check_op_cap("get_op_specific_info", "UnpadPagedAttentionMixLlm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
