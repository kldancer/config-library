...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['Q', 'Kcache', 'Vcache', 'qSeqlen', 'kvSeqlen', 'kvSeqlenShape', 'layerID', 'AttentionMask']
...
def _build_args(Q_in__, Kcache_in__, Vcache_in__, qSeqlen_in__, kvSeqlen_in__, kvSeqlenShape_in__, layerID_in__, AttentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_, kv_head, max_seqlen):
    __inputs__ = []
    for arg in [Q_in__, Kcache_in__, Vcache_in__, qSeqlen_in__, kvSeqlen_in__, kvSeqlenShape_in__, layerID_in__, AttentionMask_in__]:
        if arg != None:
    ...
    if max_seqlen != None:
        attr = {}
        attr["name"] = "max_seqlen"
        attr["dtype"] = "int"
        attr["value"] = max_seqlen
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("UnpadFlashAttentionMixSd", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def unpad_flash_attention_mix_sd(Q_in__, Kcache_in__, Vcache_in__, qSeqlen_in__, kvSeqlen_in__, kvSeqlenShape_in__, layerID_in__, AttentionMask_in__=None, outputO_out_=None, outputS_out_=None, outputP_out_=None, oTmp_out_=None, kv_head=2, max_seqlen=32768, kernel_name="unpad_flash_attention_mix_sd", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(Q_in__, Kcache_in__, Vcache_in__, qSeqlen_in__, kvSeqlen_in__, kvSeqlenShape_in__, layerID_in__, AttentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_, kv_head, max_seqlen)
    ...
    origin_func_name = "unpad_flash_attention_mix_sd"
    ascendc_src_dir_ex = "unpad_flash_attention_mix_sd"
    ascendc_src_dir = "unpad_flash_attention_mix_sd"
    ascendc_src_file = "unpad_flash_attention_mix_sd.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator UnpadFlashAttentionMixSd. kernel name is " + kernel_name)
    op_type = "UnpadFlashAttentionMixSd"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[Q_in__, Kcache_in__, Vcache_in__, qSeqlen_in__, kvSeqlen_in__, kvSeqlenShape_in__, layerID_in__, AttentionMask_in__], origin_outputs = [outputO_out_, outputS_out_, outputP_out_, oTmp_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'optional', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(Q_in__, Kcache_in__, Vcache_in__, qSeqlen_in__, kvSeqlen_in__, kvSeqlenShape_in__, layerID_in__, AttentionMask_in__=None, outputO_out_=None, outputS_out_=None, outputP_out_=None, oTmp_out_=None, kv_head=2, max_seqlen=32768, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(Q_in__, Kcache_in__, Vcache_in__, qSeqlen_in__, kvSeqlen_in__, kvSeqlenShape_in__, layerID_in__, AttentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_, kv_head, max_seqlen)
    result = check_op_cap("op_select_format", "UnpadFlashAttentionMixSd", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(Q_in__, Kcache_in__, Vcache_in__, qSeqlen_in__, kvSeqlen_in__, kvSeqlenShape_in__, layerID_in__, AttentionMask_in__=None, outputO_out_=None, outputS_out_=None, outputP_out_=None, oTmp_out_=None, kv_head=2, max_seqlen=32768, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(Q_in__, Kcache_in__, Vcache_in__, qSeqlen_in__, kvSeqlen_in__, kvSeqlenShape_in__, layerID_in__, AttentionMask_in__, outputO_out_, outputS_out_, outputP_out_, oTmp_out_, kv_head, max_seqlen)
    result = check_op_cap("get_op_specific_info", "UnpadFlashAttentionMixSd", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
