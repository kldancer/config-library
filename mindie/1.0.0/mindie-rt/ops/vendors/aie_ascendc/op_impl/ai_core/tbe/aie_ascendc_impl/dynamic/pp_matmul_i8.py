...
def _build_args(a_in__, b_in__, bias_in__, scale_in__, c_out_, transposeA, transposeB, withBias, enDequant, oriShape, tilingK, tilingN):
    __inputs__ = []
    for arg in [a_in__, b_in__, bias_in__, scale_in__]:
        if arg != None:
    ...
    if transposeA != None:
        attr = {}
        attr["name"] = "transposeA"
        attr["dtype"] = "bool"
        attr["value"] = transposeA
        __attrs__.append(attr)
    if transposeB != None:
        attr = {}
        attr["name"] = "transposeB"
        attr["dtype"] = "bool"
        attr["value"] = transposeB
        __attrs__.append(attr)
    if withBias != None:
        attr = {}
        attr["name"] = "withBias"
...
@tbe_register.register_operator("PpMatmulI8", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def pp_matmul_i8(a_in__, b_in__, bias_in__, scale_in__, c_out_, transposeA=False, transposeB=False, withBias=True, enDequant=True, oriShape=[], tilingK=0, tilingN=0, kernel_name="pp_matmul_i8", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, b_in__, bias_in__, scale_in__, c_out_, transposeA, transposeB, withBias, enDequant, oriShape, tilingK, tilingN)
    ...
    origin_func_name = "pp_matmul_i8"
    ascendc_src_dir_ex = "pp_matmul_i8"
    ascendc_src_dir = "pp_matmul_i8"
    ascendc_src_file = "pp_matmul_i8.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator PpMatmulI8. kernel name is " + kernel_name)
    op_type = "PpMatmulI8"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[a_in__, b_in__, bias_in__, scale_in__], origin_outputs = [c_out_],\
...
def op_select_format(a_in__, b_in__, bias_in__, scale_in__, c_out_, transposeA=False, transposeB=False, withBias=True, enDequant=True, oriShape=[], tilingK=0, tilingN=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, b_in__, bias_in__, scale_in__, c_out_, transposeA, transposeB, withBias, enDequant, oriShape, tilingK, tilingN)
    result = check_op_cap("op_select_format", "PpMatmulI8", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(a_in__, b_in__, bias_in__, scale_in__, c_out_, transposeA=False, transposeB=False, withBias=True, enDequant=True, oriShape=[], tilingK=0, tilingN=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, b_in__, bias_in__, scale_in__, c_out_, transposeA, transposeB, withBias, enDequant, oriShape, tilingK, tilingN)
    result = check_op_cap("get_op_specific_info", "PpMatmulI8", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
