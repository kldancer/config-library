...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['TokenA', 'TokenB', 'TOPK_Indice', 'Arg_Max']
    output_names = ['unmergeTokenA', 'unReduceTokenB', 'unReduceCount']
...
def _build_args(TokenA_in__, TokenB_in__, TOPK_Indice_in__, Arg_Max_in__, unmergeTokenA_out_, unReduceTokenB_out_, unReduceCount_out_, topRRate):
    __inputs__ = []
    for arg in [TokenA_in__, TokenB_in__, TOPK_Indice_in__, Arg_Max_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [unmergeTokenA_out_, unReduceTokenB_out_, unReduceCount_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("TomeMerged", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def tome_merged(TokenA_in__, TokenB_in__, TOPK_Indice_in__, Arg_Max_in__, unmergeTokenA_out_, unReduceTokenB_out_, unReduceCount_out_, topRRate=0.5, kernel_name="tome_merged", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(TokenA_in__, TokenB_in__, TOPK_Indice_in__, Arg_Max_in__, unmergeTokenA_out_, unReduceTokenB_out_, unReduceCount_out_, topRRate)
    ...
    origin_func_name = "tome_merged"
    ascendc_src_dir_ex = "tome_merged"
    ascendc_src_dir = "tome_merged"
    ascendc_src_file = "tome_merged.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator TomeMerged. kernel name is " + kernel_name)
    op_type = "TomeMerged"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[TokenA_in__, TokenB_in__, TOPK_Indice_in__, Arg_Max_in__], origin_outputs = [unmergeTokenA_out_, unReduceTokenB_out_, unReduceCount_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(TokenA_in__, TokenB_in__, TOPK_Indice_in__, Arg_Max_in__, unmergeTokenA_out_, unReduceTokenB_out_, unReduceCount_out_, topRRate=0.5, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(TokenA_in__, TokenB_in__, TOPK_Indice_in__, Arg_Max_in__, unmergeTokenA_out_, unReduceTokenB_out_, unReduceCount_out_, topRRate)
    result = check_op_cap("op_select_format", "TomeMerged", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(TokenA_in__, TokenB_in__, TOPK_Indice_in__, Arg_Max_in__, unmergeTokenA_out_, unReduceTokenB_out_, unReduceCount_out_, topRRate=0.5, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(TokenA_in__, TokenB_in__, TOPK_Indice_in__, Arg_Max_in__, unmergeTokenA_out_, unReduceTokenB_out_, unReduceCount_out_, topRRate)
    result = check_op_cap("get_op_specific_info", "TomeMerged", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
