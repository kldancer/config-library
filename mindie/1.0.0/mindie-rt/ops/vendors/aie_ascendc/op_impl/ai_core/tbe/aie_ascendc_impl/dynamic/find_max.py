...
def _build_args(x_in__, values_out_, indices_out_):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [values_out_, indices_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("FindMax", trans_bool_to_s8=False)
...
def find_max(x_in__, values_out_, indices_out_, kernel_name="find_max", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, values_out_, indices_out_)
    ...
    origin_func_name = "find_max"
    ascendc_src_dir_ex = "find_max"
    ascendc_src_dir = "find_max"
    ascendc_src_file = "find_max.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator FindMax. kernel name is " + kernel_name)
    op_type = "FindMax"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__], origin_outputs = [values_out_, indices_out_],\
...
def op_select_format(x_in__, values_out_, indices_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, values_out_, indices_out_)
    result = check_op_cap("op_select_format", "FindMax", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, values_out_, indices_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, values_out_, indices_out_)
    result = check_op_cap("get_op_specific_info", "FindMax", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
