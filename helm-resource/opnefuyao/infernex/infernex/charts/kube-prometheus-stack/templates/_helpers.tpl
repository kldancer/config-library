{{- define "prometheus.installed" -}}
{{- if .Capabilities.APIVersions.Has "monitoring.coreos.com/v1" -}}
  {{- $operatorFound := false -}}
  {{- $allNamespaces := (lookup "v1" "Namespace" "" "").items -}}
  {{- $namespaces := default $allNamespaces (list "monitoring" "kube-system" "default") -}}
  {{- range $namespace := $namespaces }}
    {{- if $operatorFound }}{{ continue }}{{ end }}
    {{- $pods := (lookup "v1" "Pod" $namespace "").items }}
    {{- range $pod := $pods }}
      {{- if $operatorFound }}{{ continue }}{{ end }}
      {{- $labelVal := index $pod.metadata.labels "app.kubernetes.io/name" }}
      {{- if $labelVal }}
        {{- if contains $labelVal "prometheus-operator" }}
          {{- $operatorFound = true }}
        {{- end }}
      {{- end }}
    {{- end }}
  {{- end }}
  {{- $operatorFound | toString -}}
{{- else -}}
  false
{{- end -}}
{{- end -}}

{{- define "npu.exporter.installed" -}}
{{- $operatorFound := false -}}
{{- $allNamespaces := (lookup "v1" "Namespace" "" "").items -}}
{{- $namespaces := default $allNamespaces (list "npu-exporter") -}}
{{- range $namespace := $namespaces }}
  {{- $pods := (lookup "v1" "Pod" $namespace "").items -}}
  {{- range $pod := $pods }}
    {{- $labelVal := index $pod.metadata.labels "app" }}
    {{- if and (eq $labelVal "npu-exporter") (eq $pod.status.phase "Running") }}
      {{- $operatorFound = true -}}
    {{- end }}
  {{- end }}
{{- end }}
