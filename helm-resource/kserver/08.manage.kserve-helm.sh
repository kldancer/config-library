...
USE_LOCAL_CHARTS="${USE_LOCAL_CHARTS:-true}"
...
install() {
    ...
    if [ "${USE_LOCAL_CHARTS}" = true ]; then
        ...
        sed -i -e "s/*defaultVersion*/${KSERVE_VERSION}/g" package/06-kserve/kserve/values.yaml
            
        # Install KServe CRDs from local chart
        log_info "Installing KServe CRDs..."
        helm upgrade -i "${KSERVE_CRD_RELEASE_NAME}" "package/06-kserve/kserve-crd" \
        ...
        helm upgrade -i "${KSERVE_RELEASE_NAME}" "package/06-kserve/kserve" \
            --namespace "${KSERVE_NAMESPACE}" \
            --create-namespace \
            --wait \
            -f package/06-kserve/kserve/values.yaml \
            ${KSERVE_EXTRA_ARGS:-}
