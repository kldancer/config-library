...
ENVOYPROXY_NAME="${GATEWAYCLASS_NAME:-kserve-proxy-private}"
# VARIABLES END
...
uninstall() {
    ...
    log_info "Deleting EnvoyProxy '${ENVOYPROXY_NAME}'..."
    kubectl delete envoyproxy "${ENVOYPROXY_NAME}" --ignore-not-found=true --force --grace-period=0 2>/dev/null || true
...
install() {
    ...
    # 自定义EnvoyProxy镜像
    log_info "Creating EnvoyProxy 'kserve-proxy-private'..."
